...
def download_playlists(yt_dlp_path, playlist_urls, download_archive_path, download_dir):
    """Letölti a megadott YouTube lejátszási listákat az eredeti audioformátumban."""
    for url in playlist_urls:
        ...
        cmd = [
            yt_dlp_path,
            '-f', 'bestaudio/best',    # Az eredeti (legtöbbször opus/m4a) formátumban marad
            '--no-overwrites',
            '--restrict-filenames',
            url,
            '-o', output_template,
        ...
        # 1–2 másodperc véletlenszerű szünet beiktatása
        pause = random.uniform(1, 2)
        print(f"Várakozás {pause:.1f} másodpercig a következő letöltés előtt...")
        time.sleep(pause)

def main():
    parser = argparse.ArgumentParser(description="YouTube lejátszási listák letöltése az eredeti audioformátumban.")
    ...
    parser.add_argument('--playlists_file', type=str, required=True, help='Szövegfájl, amely a lejátszási lista URL-eket tartalmazza (egy sorban egy URL)')
    ...
    # Telepítjük az ffmpeg-et (ha szükséges)
    install_ffmpeg()
    ...
    print("Minden lejátszási lista letöltése befejeződött.")
